        ...
        <div class="contact-container px-5 container mx-auto">
            <ul class="flex flex-col md:flex-row items-center justify-center gap-3 md:gap-10">
                <li class="text-md font-semibold flex align-center items-center mb-3 md:mb-0 ml-1 md:ml-3">
                    <img class="h-5 md:h-7" src="{{ asset('assets/svg/chat.svg') }}" alt="{{ __('Omniscan - chat pictogram') }}">
                    <p class="ml-3">{{ __('Contact Us') }} : </p>
                </li>
                <li class="text-md flex align-center items-center mb-3 md:mb-0">
                    <img class="h-3 md:h-5" src="{{ asset('assets/svg/mail.svg') }}" alt="{{ __('Omniscan - mail pictogram') }}">
                    <a href="mailto:{{ config('app.mail') }}" class="ml-3" title="{{ config('app.mail') }}">{{ config('app.mail') }}</a>
                </li>
                <li class="text-md flex align-center items-center">
                    <img class="h-4 md:h-6" src="{{ asset('assets/svg/phone.svg') }}" alt="{{ __('Omniscan - phone pictogram') }}">
                    <a href="tel:{{ config('app.phone') }}" class="ml-3" title="{{ config('app.phone') }}">{{ config('app.phone') }}</a>
                </li>
            </ul>
        </div>
    </div>
